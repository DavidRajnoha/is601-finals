...
async def test_last_login_update(db_session: AsyncSession, user: User):
    ...
    await db_session.commit()
    await db_session.refresh(user)
    expected_naive_last_login = new_last_login.replace(tzinfo=None)

    assert user.last_login_at == expected_naive_last_login, "Last login timestamp should update correctly"
